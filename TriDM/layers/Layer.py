...
class WeightGenerator(nn.Module):
    ...
    def forward(self):
        if self.factorized:# no need to generate weights if factorized
            ...
            return weights, bias#解释一下，weights是一个列表，里面包含了四个权重矩阵，bias是一个列表，里面包含了四个偏置向量
        else:
