...
class SparseDispatcher(object):#分配样本到门控值非零的专家
    ...
    def dispatch(self, inp):#将输入样本分配给那些门控值非零的专家
    ...
    def combine(self, expert_out, multiply_by_gates=True):
        ...
        # back to log space
        return combined.log()#解释：将分配给各个专家的样本输出进行指数运算，然后根据门控值进行加权，最后将结果返回到对数空间。
...
class series_decomp_multi(nn.Module):
    ...
    def forward(self, x):
        moving_mean = []
        for func in self.moving_avg:#列表循环加权的移动平均值
            moving_avg = func(x)
...
class FourierLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        b, t, d = x.shape
        x_freq = fft.rfft(x, dim=1)#B，seq//2+1，N

        if t % 2 == 0:
            x_freq = x_freq[:, self.low_freq:-1]#去掉最高频和最低频
            f = fft.rfftfreq(t)[self.low_freq:-1]
        else:
            x_freq = x_freq[:, self.low_freq:]
            f = fft.rfftfreq(t)[self.low_freq:]

        x_freq, index_tuple = self.topk_freq(x_freq)#B，topk，N
        f = repeat(f, 'f -> b f d', b=x_freq.size(0), d=x_freq.size(2))
        f = f.to(x_freq.device)
        f = rearrange(f[index_tuple], 'b f d -> b f () d').to(x_freq.device)#B，topk，1，N

        return self.extrapolate(x_freq, f, t), None

    def extrapolate(self, x_freq, f, t):
        x_freq = torch.cat([x_freq, x_freq.conj()], dim=1)#拼接自己的共轭复数
        f = torch.cat([f, -f], dim=1)#拼接负数
        t_val = rearrange(torch.arange(t + self.pred_len, dtype=torch.float),
                          't -> () () t ()').to(x_freq.device)

        amp = rearrange(x_freq.abs() / t, 'b f d -> b f () d')#计算 x_freq 的幅度，除以序列长度 t 进行归一化
        phase = rearrange(x_freq.angle(), 'b f d -> b f () d')#计算 x_freq 的相位

        x_time = amp * torch.cos(2 * math.pi * f * t_val + phase)

        return reduce(x_time, 'b f t d -> b t d', 'sum')

    def topk_freq(self, x_freq):#从频率表示 x_freq 中选择幅度最大的 k 个频率分量
